...
$(function () {
		...
		$.get(content, function (response) {
			...
			setTimeout(function () {
				...
				if (/product/ig.test(content)) {
					...
					$(span).html(c);
					$(a).attr('class', 'back vertical-middle fontsize-30 fontsize-xs-36').append(span).attr('href', '#').on('click', function () {
			...
			$('<a href=\'#\' class=\'top fade\'><img src=\'' + rootPath + 'img/common/top.png\'></a>').appendTo($('#content .inner'));
	...
	$('header nav.menu li[data-content], header nav.menu a[data-content]').on('click', function () {
		...
		var content = rootPath + $(this).attr('data-content') + '/',
		    cat = $(this).attr('data-cat'),
		    cata = $(this).attr('data-cata');
		// console.log(content);
	...
	function prepareImage() {
		...
		function imageLoaded() {
			...
			$('.blend').css('background-image', 'url(' + $('.blend').attr('data-src') + '), url(' + $(litMotor).attr('data-src') + ')');
		}
	}
	prepareImage();
	...
	function kvFreeze() {
		...
		var backgroundImage = currentKv.css('background-image') != 'none' ? currentKv.css('background-image') : 'url(' + currentKv.attr('data-src') + ')';
	...
	if (getParam('content') && getParam('cat') && getParam('cata')) {
		...
		var content = rootPath + getParam('content') + '/',
		    cat = getParam('cat'),
		    cata = getParam('cata');
		// console.log(content);
